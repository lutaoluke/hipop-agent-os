...
_DESCRIPTIVE_DENOM_RE = re.compile(
    ...
    # 以 产品/noon 售价/价格 为/作为 计算/核算基准/基础/基数
    r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价|价格)\s*为\s*(?:计算|核算)?(?:基准|基础|基数)'
    r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*为\s*(?:计算|核算)?(?:基准|基础|基数)'
    r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价|价格)\s*作为\s*(?:计算|核算)?(?:基准|基础|基数)'
    r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*作为\s*(?:计算|核算)?(?:基准|基础|基数)'
    # 按 产品/noon 售价/价格 作为 基准来算
    r'|按\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价|价格)\s*作为\s*(?:计算|核算)?(?:基准|基础|基数)(?:来?算|计算)?'
    r'|按\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*作为\s*(?:计算|核算)?(?:基准|基础|基数)(?:来?算|计算)?'
...
_PLUS_STILL_IN_KPI_RE = re.compile(
    ...
    r'plus.{0,80}(?<!不)(?:计入|纳入|算入|算进|进入|归入|归到|列入|计到|算作|算).{0,20}(?:采购端|采购|议价)?(?:绩效|KPI|考核|考核项|指标|目标)'
    ...
    # "绩效考核时 仍 纳入/计入/进入/归入"
    r'|绩效考核时?\s*仍\s*(?:纳入|计入|进入|归入|归到).{0,15}(?:绩效|KPI|考核|指标|目标)'
    # "要把 plus ... 算进去/计入/进入/归入"
    r'|要把.{0,5}plus.{0,30}(?:一起)?.{0,5}(?:算进去|算进来|算入|计入|纳入|进入|归入|归到)'
