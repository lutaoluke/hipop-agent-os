...
# 错误分母（自然话术）：以"分母用 X"/"以 X 作为分母"/"用 X 做分母"/"计算基数用X"等方式引入错误价格基准。
# 补充 _FORMULA_EXTERNAL_DENOM_RE 仅覆盖"议价差额 ÷ X"公式写法的盲区。
_DESCRIPTIVE_DENOM_RE = re.compile(
    # 分母用/是/为/按/采用/设为 noon/产品/商品/平台 ... 售价/价格
...
_PLUS_STILL_IN_KPI_RE = re.compile(
    # "但计入/纳入/进入/归入/计到/算作 采购绩效/KPI/考核"（无 会/仍/还 等 modal 词）
    ...
    # "plus 补贴/折扣 作为 采购绩效/KPI/考核 核算/指标"
