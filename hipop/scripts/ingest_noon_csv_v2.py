...
- `run_live(...)` / `set_live_row_producer` / `get_live_row_producer` / `LiveSourceUnavailable`
  + CSV 回落
- **本模块只定 socket + 行形状契约，不实现真抓取**（真 producer 归 WS-N2.1/WS-58）。
  行字段以 WS-34 noon 订单行契约为唯一来源（键同 noon 订单 CSV 列），不自定义。
"""
...
class LiveSourceUnavailable(Exception):
    ...
    空订单 / 默认销量金额当成功落库（占位假数据死法）。
    """


# WS-N2.1/WS-58 noon 订单 live row producer 接入点。fetcher land 后调
# set_live_row_producer(fn) 注册；fn(tenant_id) -> Iterable[dict]，产出
# **同形 dict row**（键同 noon 订单 CSV 列：partner_sku / sku / item_nr /
# order_timestamp / status / fulfillment_model / offer_price / gmv_lcy /
# currency_code / dest_country / family / brand_code ...，见 WS-34 行契约）。
# 本任务不猜抓取字段，只定 row 形状契约；未注册（fetcher 未 land）→ run_live 回落 CSV。
...
def _aggregate(rows, tenant_id, entity_alias=None):
    ...
    `rows` 是 dict 的可迭代对象（键同 noon 订单 CSV 列，WS-34 契约）；CSV 入口经
    `_iter_csv_rows` 解析后喂这里，live 源直接喂同形 row，二者共用同一聚合 +
    同一 `_upsert`，落库结果逐字段一致（不分叉）。
...
def run_live(tenant_id: int, live_producer=None, allow_csv_fallback: bool = True,
    ...
    读：noon 订单 live row producer（WS-N2.1/WS-58 接入；未接入则回落 CSV interim）；
