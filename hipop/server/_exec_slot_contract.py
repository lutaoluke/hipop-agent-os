...
# Form A —— 任务/工作流主语（执行实体）。
_EXEC_SUBJECT_RE = re.compile(r"任务|工作流|后台任务|后台流程|流程|后台")

# Form B —— 执行动作闭集（系统/工作流有限操作；领域建模，**不含趋势词**）。
# 趋势词（改善/回升/下滑/好转/回暖/走低/走高/增长/下降/上升/回落/企稳/放缓/恶化/向好）
# 不在此集 → 「周转已开始改善 / 销量已开始回升」天然不命中。
_EXEC_ACTION_RE = re.compile(
    r"刷新|刷库存?|重算|重新计算|重跑|重新跑|同步|执行|启动|触发|创建|提交|受理|"
    r"采集|抓取|入库|拉取|拉数|扫描|扫一下|生成|重置|录入|跑完|跑好|算完"
)

# 启动/完成/进行体（aspect：声称「已经/正在/已完成做了某动作」）。
_ASPECT_RE = re.compile(
    r"已开始|已启动|已触发|已提交|已受理|已创建|已完成|完成了|已完毕|完毕|"
    r"已在[^，。！？!?；;、,\n]{0,4}后台|已经在[^，。！？!?；;、,\n]{0,4}(?:后台|跑)|"
    r"后台[^，。！？!?；;、,\n]{0,4}(?:跑|运行)了|正在|进行中|"
    r"已执行|已重新计算|已重算|已刷新|已同步|已生成|已采集|已抓取|已入库|已录入|"
    r"跑完了?|重算[^，。！？!?；;、,\n]{0,3}(?:完|好|了)"
)
...
def is_exec_claim(clause: str) -> bool:
    ...
    if _FAKE_TASK_EVIDENCE_RE.search(clause):
        return True
    if not _ASPECT_RE.search(clause):
        return False
    # 有启动/完成体 + (执行实体主语 或 执行动作闭集动词) → 执行声明
    return bool(_EXEC_SUBJECT_RE.search(clause) or _EXEC_ACTION_RE.search(clause))
