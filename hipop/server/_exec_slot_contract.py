...
# ── 体（aspect）—— **compositional 结构判别，不枚举具体短语**（route-b round-3 打回根因 =
#    枚举「已开始」漏「已经开始」的打地鼠）。已/已经/正/正在/刚/刚刚 由「已经?|正在?|刚刚?」
#    一次性覆盖口语变体，不再逐个加。
_ASP_PRE = r"(?:已经?|正在?|刚刚?)(?:开始)?"   # 体在动词前：已/已经/正在/刚 [+开始]
# 完成/启动体（动词后 或 完成体引导）。**强制带「已」或「了」**，避开「完成度/成功率/集中/
# 中文」这类无完成 aspect 标记的假友。
_ASP_DONE = (
    r"(?:(?:已经?|刚刚?)(?:开始|启动|完成|完毕|提交|创建|受理|触发)"
    r"|(?:开始|完成|完毕|好|成功)了)"
)
# 0–2 个数据对象（库存/销量/物流…）作为体与动词之间的**闭集间隔**——不是任意字符（任意字符会
# 像 round-1 那样跨「改善并建议」重新引入 FP）。
_OBJ_GAP = r"(?:" + _DATA_OBJ + r"){0,2}"

# 执行声明 = **体直接绑定执行动词**（贴邻 / 仅隔闭集数据对象），双向语序一次覆盖：
#   A. 体在前 + (对象) + 动词：已刷新 / 已经开始库存刷新 / 正在重算
#   B. 动词 + 体在后：刷新已开始 / 刷新已经开始 / 重算已完成 / 同步完成了
#   C. 完成体 + (对象) + 动词：已完成库存刷新 / 已经完成重算
#   D. 已(经)在…后台 + 跑/运行/执行/处理
# 趋势词（改善/回升…不在执行动词闭集）与建议语气（建议/可/应 执行，执行前非体）→ 天然不命中，
# round-1/2 的无逗号「趋势+建议」FP 保持闭合（间隔走闭集对象、不跨趋势动词+并建议）。
_EXEC_BOUND_RE = re.compile(
    _ASP_PRE + _OBJ_GAP + r"(?:" + _EXEC_VERBS + r")"             # A
    + r"|(?:" + _EXEC_VERBS + r")" + _ASP_DONE                    # B
    + r"|" + _ASP_DONE + _OBJ_GAP + r"(?:" + _EXEC_VERBS + r")"   # C
    + r"|已经?在[^，。！？!?；;、,\n]{0,4}后台[^，。！？!?；;、,\n]{0,6}(?:跑|运行|执行|处理|算)"  # D
    + r"|后台[^，。！？!?；;、,\n]{0,4}(?:跑|运行)了"
)

# Form A 用的启动/完成体（仅与强执行主语 任务/工作流 共现时才算；覆盖「任务已完成/任务已创建/
# 任务正在执行」这类完成态动词不在执行动词集、靠主语绑定的句子）。同样 compositional。
_ASPECT_RE = re.compile(_ASP_DONE + r"|正在|进行中")
