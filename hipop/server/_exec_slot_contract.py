...
# 语气，二者不绑定）。改为要求体与执行动词**贴邻**：
#   · 已/正在/已开始 + <执行动词>      —— 已刷新 / 正在重算 / 已开始执行
#   · <执行动词> + 完成/完毕/中/成功    —— 重算完成 / 刷新中
#   · 已(经)在…后台 + 跑/运行/执行/处理 —— 已在后台跑
# 「建议执行 / 可执行 / 应执行」里执行前是建议语气词、非启动体 → 不命中。
_EXEC_BOUND_RE = re.compile(
    r"(?:已经?|正在)(?:开始)?(?:" + _EXEC_VERBS + r")"
    r"|已开始(?:" + _EXEC_VERBS + r")"
    r"|(?:" + _EXEC_VERBS + r")(?:完成|完毕|中|成功了?)"
    r"|已经?在[^，。！？!?；;、,\n]{0,4}后台[^，。！？!?；;、,\n]{0,6}(?:跑|运行|执行|处理|算)"
    r"|后台[^，。！？!?；;、,\n]{0,4}(?:跑|运行)了"
)
