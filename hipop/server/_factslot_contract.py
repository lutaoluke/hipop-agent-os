...
# 库存数量 token：带计量单位的数字，或四仓/库存语境标签后的数字（标签是有限领域集，
# 非穷举措辞）。比 _safety._QTY_RESULT_RE 多覆盖"义乌 888 / noon 111"这类无单位的拆仓数字。
_STOCK_LABELS = [
...
def scrub_fabricated_slots(
    ...
    # ── 库存数量（slot-aware 绑定校验，路线 b）────────────────────────────────────
    # 成功 → 不只是"值出现过"，而是"值-槽绑定"必须与工具一致：
    #   "总库存 N" 的 N 必须 == 工具 total；"义乌 N" 的 N 必须 == 工具 yiwu …
    #   这正是 3 轮红队卡的「总库存 509/义乌 200」被写成「总库存 200」那类跨槽搬运。
    #   有槽标签 → 按绑定值校验；无标签的裸 "N 件" → 退化为"值 ∈ 全集"包含关系。
    # 纯失败/空 → 全删。仅在无成功物流查询时启用（成功物流合法产出"在途 N"计数，避免误删）。
    stock_value_allow = {
        int(v) for e in evs
    ...
    for e in evs:
        if e.get("tool") in _STOCK_TOOLS and e.get("ok") and e.get("has_stock_value"):
            for kw, v in (e.get("stock_bind") or {}).items():
                stock_bind[kw] = v
    # 标签按长度降序匹配，避免"国内仓"被"国内"/"沙特一号仓"被"沙特"短匹配错绑
    _bind_keys = sorted(stock_bind.keys(), key=len, reverse=True)
    if stock_called and not logistics_proven:
        if stock_proven:
            def _qty_sub(m: re.Match) -> str:
                ...
                if not nums:
                    return seg
                bound = next((stock_bind[k] for k in _bind_keys if k in seg), None)
                if bound is not None:
                    if all(n == bound for n in nums):
                        return seg  # 值-槽绑定正确 → 放行
                    redacted["qty"] += 1
                    return _REDACT_QTY      # 跨槽搬运（值真关系假）→ 删
                if all(n in stock_value_allow for n in nums):
                    return seg  # 无标签裸数字且值来自工具 → 放行
                redacted["qty"] += 1
                return _REDACT_QTY
            reply = _STOCK_QTY_RE.sub(_qty_sub, reply)
        elif stock_blocked:
            def _qty_sub_fail(m: re.Match) -> str:
                redacted["qty"] += 1
                return _REDACT_QTY
            reply = _STOCK_QTY_RE.sub(_qty_sub_fail, reply)
