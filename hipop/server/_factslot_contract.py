...
返工口径——不只是"前置一段模板 + 加 warning"，而是**真的把答案正文里编造的结果槽值删掉**：
  ...
  2. 回答结构（sanitize_reply 入口，第一层）：
     a) scrub_fabricated_slots —— **包含关系判别**：reply 里出现的承运商（闭集实体）/
        运单号（id 形 token）/库存数量（工具 stock_values）/状态（状态桶）四类事实槽都必须
        ∈ 工具实际返回的对应集合，否则就是编造，**就地从正文删除该值**（不是只加 banner）。
        失败/空分支 allow-set 为空 → 全删；成功分支只删工具没给出的"多编的那个"（验收 2 的
        对称面墙——成功路径与失败路径同一套证据契约）。
...
def factslot_evidence_from_result(tool_name: str, result) -> Optional[dict]:
    ...
    if tool_name == "query_order_live":
        ...
        if ok and result.get("total") is not None:
            ...
            # 库存成功：把所有工具实际返回的数值收进 allow-set（成功分支数量校验用）
            split = result.get("split") or {}
            cand = [result.get("total"), result.get("erp_in_transit")]
            if isinstance(split, dict):
                cand.extend(split.values())
            for v in cand:
                if isinstance(v, bool):
                    continue
                if isinstance(v, int):
                    ev["stock_values"].append(v)
                elif isinstance(v, float) and v.is_integer():
                    ev["stock_values"].append(int(v))
    ...
    if stock_called and not logistics_proven:
        if stock_proven:
            def _qty_sub(m: re.Match) -> str:
                ...
                if nums and all(int(n) in stock_value_allow for n in nums):
                    return m.group(0)  # 数值都来自工具 → 放行
                redacted["qty"] += 1
                return _REDACT_QTY
