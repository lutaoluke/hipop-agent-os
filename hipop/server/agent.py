...
**3b. 用户问"刷新 / 跑工作流 / 同步数据 / 重算 X / 扫 ERP / 拉数据"时，必须本轮**真的**调 `run_workflow`，禁止只口头描述**
   ...
   - **死规矩 · 查询/拉取声明**：说出"我查了/已查/拉了/已拉 [商品/SKU/数据/货单]"等完成态 ⇔ 本轮必须有 `list_products(limit>0)` / `query_sku` / `query_order` 对应调用；`list_products(limit=0)` 只是计数，不能用来证明"我查了"，如实说"我看了总数是 X 个"而非"我查了商品"。
...
**6. 表格字段必须用真实存在的列**
   ...
   - 现有 wf2 字段：`partner_sku / title / sales_10d / sales_30d / sales_60d / sales_90d / sales_180d / latest_price / latest_profit_rate / is_listed / sales_grade`；query_sku 工具额外返回 `total_orders_30d`（30d 窗口总单）/ `cancel_rate_30d`（30d 取消率）/ `return_rate_30d`（30d 退货率）/ `history_total`（ERP 历史总单）/ `as_of_date`（数据口径截止日）
...
def chat(messages: List[Dict], scope: Dict) -> Dict:
    ...
    if final_text and (refs_collected or tool_log):
        try:
            action_id = _data.write_agent_action(
                ...
                judge=judge,
                pill_text="执行" if _is_substantive_action(tool_log) else ("查询" if tool_log else "信息"),
                pill="info",
    ...
    return {
        ...
        "tag": ("hallucinate" if hallu_warnings else ("执行" if _is_substantive_action(tool_log) else ("查询" if tool_log else None))),
        "workflow_task": workflow_task,
        "provider": _provider.get_provider(),
