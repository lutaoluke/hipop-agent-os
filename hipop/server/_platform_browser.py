...
继承 contextvar，不重设。当前 account/凭据来自 `ziniao_client` 配置；后续接
`tenant_*_credentials` 加密表（像 ERP 的 _crypto + RLS）。
...
def get_browser_list(account: Optional[str] = None, *, port: Optional[int] = None) -> list:
    """调本机 `getBrowserList`，返回原始 store 条目列表。

    请求 body 必含 company/username/password/action/requestId（紫鸟鉴权所需）。
    ...
    body = {
        ...
        "username": account or cfg.get("username"),
...
def list_stores(account: Optional[str] = None, *, port: Optional[int] = None) -> list:
    """枚举 account 下**全部** store（不硬编 browserId）。

    每个 Store 携带后续 tenant/entity 映射所需字段：browser_id、store_username、name。
    返回 [] 表示该 account 没有任何 store（调用方/select_store 据此 report blocked）。
    """
    acct = account or (_client_cfg().get("username") or "")
    entries = get_browser_list(account=account, port=port)
    return [_norm_store(e, acct) for e in entries if isinstance(e, dict)]
...
def open_cdp_endpoint(store_key, account: Optional[str] = None, *,
    ...
    connect_over_cdp / 登录态检测 / 三级回落（WS-33.2/33.3）。
    """
    stores = list_stores(account=account, port=port)
    if not stores:
        raise PlatformBrowserError(
