...
def test_failed_workflow_named_but_claimed_success_adds_banner():
    """round-9 红队：reply 点名了失败 workflow，但同时声称其"成功"→ 必须加 banner。

    Fail-then-pass：旧代码（_check_failed_workflow_claimed_success 只看"未点名"）
    对 | wf2_sales_v2 | 成功 | 返回 warns=[]（放行假成功表格）。
    新代码（_workflow_claimed_succeeded 同行失败语言检测）必须返回非空 warns。
    """
    tool_log = [{"name": "run_workflow", "args": {"workflow": "wf2_sales_v2"},
                 "ok": False, "task_id": None, "error": "permission_denied"}]

    # 红队 1：Markdown 表格行，状态列"成功"
    table_reply = "| 工作流 | 状态 |\n| --- | --- |\n| wf2_sales_v2 | 成功 |"
    warns_table = _safety._check_failed_workflow_claimed_success(table_reply, tool_log)
    assert warns_table, f"表格假成功应加 banner: {warns_table!r}"
    assert "wf2_sales_v2" in warns_table[0], f"banner 应含失败工作流名: {warns_table[0]}"

    # 红队 2：内联"已启动 wf2_sales_v2"
    inline_reply = "已启动 wf2_sales_v2，请等待完成。"
    warns_inline = _safety._check_failed_workflow_claimed_success(inline_reply, tool_log)
    assert warns_inline, f"内联已启动假成功应加 banner: {warns_inline!r}"

    # 正面：reply 点名了 workflow 且明确说"失败" → 不加 banner
    honest_reply = "wf2_sales_v2 启动失败（permission_denied），请检查权限。"
    warns_honest = _safety._check_failed_workflow_claimed_success(honest_reply, tool_log)
    assert not warns_honest, f"如实说失败不应加 banner: {warns_honest!r}"

    # 正面：混合结果，一成一败，reply 正确描述 wf2_sales_v2 失败 → 不加 banner
    mixed_reply = "ERP 商品库刷新成功（任务 abc12345），wf2_sales_v2 启动失败：permission_denied。"
    warns_mixed = _safety._check_failed_workflow_claimed_success(mixed_reply, tool_log)
    assert not warns_mixed, f"混合结果如实描述失败不应加 banner: {warns_mixed!r}"
...
if __name__ == "__main__":
    tests = [
        ...
        test_failed_workflow_named_but_claimed_success_adds_banner,
