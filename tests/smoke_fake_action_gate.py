...
from hipop.server import _provider as _agent_provider
...
# ── 第9轮：list_products 省略 limit 必须按真实默认 0 处理 ───────────────────

def test_fake_query_omitted_limit_dict_caught():
    """Anthropic dict args 省略 limit → list_products 默认 limit=0，只计数，声称查商品必须拦。"""
    tool_log = [{"name": "list_products", "args": {"store": "KSA"}}]
    _, warns = _safety.sanitize_reply(
        "我查了你的商品，总共有 1798 个 SKU。", ["list_products"], tool_log=tool_log
    )
    assert any("hallucinate" in w or "list_products" in w or "查询" in w for w in warns), \
        f"省略 limit 的 dict args 被当成真实查询证据，漏拦: {warns}"


def test_fake_query_omitted_limit_gpt_string_caught():
    """GPT raw string args 省略 limit → list_products 默认 limit=0，只计数，声称查商品必须拦。"""
    tool_log = [{"name": "list_products", "args": '{"store":"KSA"}'}]
    _, warns = _safety.sanitize_reply(
        "我查了你的商品，总共有 1798 个 SKU。", ["list_products"], tool_log=tool_log
    )
    assert any("hallucinate" in w or "list_products" in w or "查询" in w for w in warns), \
        f"省略 limit 的 GPT string args 被当成真实查询证据，漏拦: {warns}"


def test_is_substantive_action_omitted_limit_dict_false():
    """Anthropic dict args 省略 limit → _is_substantive_action 返回 False。"""
    tool_log = [{"name": "list_products", "args": {"store": "KSA"}}]
    assert not _safety._is_substantive_action(tool_log), \
        "省略 limit 应按 list_products 默认 limit=0 处理，返回 False"


def test_is_substantive_action_omitted_limit_gpt_string_false():
    """GPT raw string args 省略 limit → _is_substantive_action 返回 False。"""
    tool_log = [{"name": "list_products", "args": '{"store":"KSA"}'}]
    assert not _safety._is_substantive_action(tool_log), \
        "GPT string args 省略 limit 应按默认 limit=0 处理，返回 False"


def test_is_substantive_action_string_numeric_limit_positive():
    """limit 可能是字符串数字，比较前必须 int 化。"""
    tool_log = [{"name": "list_products", "args": {"store": "KSA", "limit": "20"}}]
    assert _safety._is_substantive_action(tool_log), \
        "字符串数字 limit='20' 应返回 True"


# ── 第10轮：GPT raw JSON string args 必须仍写 agent action pill ─────────────

def _run_chat_with_fake_tool_log(tool_log):
    calls = []
    old_chat_with_tools = _agent_provider.chat_with_tools
    old_get_provider = _agent_provider.get_provider
    old_write_agent_action = _data.write_agent_action

    def fake_chat_with_tools(**_kwargs):
        return {
            "reply": "店铺共有 1798 个 SKU。",
            "tool_log": tool_log,
            "refs_collected": [],
        }

    def fake_write_agent_action(**kwargs):
        calls.append(kwargs)
        return "action-smoke"

    _agent_provider.chat_with_tools = fake_chat_with_tools
    _agent_provider.get_provider = lambda: "smoke"
    _data.write_agent_action = fake_write_agent_action
    try:
        result = _agent.chat(
            [{"role": "user", "content": "商品总数"}],
            {"store": "KSA", "tenant_id": 1, "current_user": "Smoke"},
        )
    finally:
        _agent_provider.chat_with_tools = old_chat_with_tools
        _agent_provider.get_provider = old_get_provider
        _data.write_agent_action = old_write_agent_action
    return result, calls


def test_chat_gpt_string_args_limit_positive_writes_execute_action():
    """GPT raw string args limit>0 → write_agent_action 被调用，pill_text=执行。"""
    tool_log = [{"name": "list_products", "args": '{"store":"KSA","sku":"ABC","limit":20}'}]
    result, calls = _run_chat_with_fake_tool_log(tool_log)
    assert result["action_id"] == "action-smoke", f"raw string args 未写 action: {result}"
    assert result["tag"] == "执行", f"limit>0 tag 应为执行: {result}"
    assert calls and calls[0]["pill_text"] == "执行", f"pill_text 应为执行: {calls}"
    assert calls[0]["subject"] == "ABC", f"raw string args subject 未正确解析: {calls}"


def test_chat_gpt_string_args_limit_zero_writes_query_action():
    """GPT raw string args limit=0 → write_agent_action 被调用，pill_text=查询。"""
    tool_log = [{"name": "list_products", "args": '{"store":"KSA","limit":0}'}]
    result, calls = _run_chat_with_fake_tool_log(tool_log)
    assert result["action_id"] == "action-smoke", f"raw string args 未写 action: {result}"
    assert result["tag"] == "查询", f"limit=0 tag 应为查询: {result}"
    assert calls and calls[0]["pill_text"] == "查询", f"pill_text 应为查询: {calls}"
...
if __name__ == "__main__":
    tests = [
        ...
        test_fake_query_omitted_limit_dict_caught,
        test_fake_query_omitted_limit_gpt_string_caught,
        test_is_substantive_action_omitted_limit_dict_false,
        test_is_substantive_action_omitted_limit_gpt_string_false,
        test_is_substantive_action_string_numeric_limit_positive,
        test_chat_gpt_string_args_limit_positive_writes_execute_action,
        test_chat_gpt_string_args_limit_zero_writes_query_action,
    ]
    failed = 0
    for t in tests:
        try:
