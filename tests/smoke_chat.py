...
_STALE_RE = (
    # ① 旧 在前、数据名词在后（旧数据 / 旧的口径 / 旧 noon 数据）；间隔内不得有产品对象，
    #    所以 "旧机型数据" 不算（旧 后面紧跟产品对象 机型，够不到数据名词）。
    rf"(?:{_STALE_DEG})?旧(?:(?!{_STALE_OBJ}|旧).){{0,10}}?(?:数据|销量|库存|口径|快照)"
    # ② 数据名词在前、旧在后（数据偏旧 / 同步时间较旧 / 数据是 5/5 的（偏旧））；左侧间隔
    #    不含产品对象，**右侧紧跟也不得是产品对象**（对称封死 库存旧机型 / 口径旧版）。
    rf"|(?:{_STALE_DATA})(?:(?!{_STALE_OBJ}|旧).){{0,15}}?(?:{_STALE_DEG})?旧(?!的?(?:{_STALE_OBJ}))"
    # ③ 非"旧"族的陈旧 / 保守说法。
    r"|偏保守|过期|不新鲜|滞后|未更新"
)
