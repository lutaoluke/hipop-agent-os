...
CASES: List[Case] = [
    ...
    # ─── 用户坚持用旧数据 ───
    # WS-55：实跑两次，Agent 都报了陈旧，但措辞每次不同（"偏旧" / "有些旧"）——deepseek
    # 自然语言抖动。旧的固定词表（陈旧/过期/…）漏掉这些合法说法 → 门口径误报（漏判
    # Agent 正确行为），且逐一补同义词是打地鼠。改用 "旧" 词干兜住（陈旧/偏旧/有些旧/
    # 较旧 全含）+ 原有其它说法。门仍**要求**出现陈旧警示（完全不提仍红）—— 修门误报，
    # 非挖空。
    Case(
        name="用户拒绝刷新（要警示陈旧 + 给答案）",
        question="不用上传 不用刷新 现在就告诉我哪些要补",
        must_contain=[r"旧|偏保守|过期|不新鲜|滞后|未更新"],   # 任一陈旧警示（"旧" 词干兜住多种措辞）
    ),
    # ─── 时间戳精度（防编精确时间）───
    Case(
