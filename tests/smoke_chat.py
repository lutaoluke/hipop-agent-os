...
CASES: List[Case] = [
    ...
    Case(
        ...
        must_not_contain=[
            # `(?<!\d:)` 排除时间戳分/秒位误伤（同 STALE case，WS-159 返工同类收紧）：
            # 仍禁泄漏 13/48/51 这些旧/错指标值，但放过 `HH:MM:13` 等时间里的秒位。
            r"(?<!\d:)\b13\b",
            r"(?<!\d:)\b48\b",
            r"(?<!\d:)\b51\b",
            r"1[,，]?967",
    ...
    Case(
        ...
        must_not_contain=[
            # 旧 stale 值 13/290 不得泄漏。`(?<!\d:)` 排除时间戳分/秒位（如快照时间
            # `00:38:13` 的 `:13`）误伤 —— 那是数据“几点的快照”，不是泄漏的销量值。
            # WS-159 返工：验门人复跑时 reply 回显 `2026-06-10 00:38:13`，秒位 13 撞了
            # 裸 `\b13\b`（脆性断言，WS-55/128 同类教训），按结构判别收紧、不放过真泄漏。
            r"(?<!\d:)\b13\b",
            r"\b290\b",
            r"取消率.*0\.0%|退货率.*0\.0%",
        ],
    ...
    # ─── WS-150: 飞书确定性拒绝 ───
    Case(
        name="WS-150: 飞书确定性拒绝（不能主动发飞书/通知群）",
        question="帮我发飞书通知大家",
        # 不调任何工具——飞书拒绝路由在 chat() 入口前端拦下，不进 LLM 自由发挥
        must_use_tools=[],
        # 必须说不支持（不是工具返回，是确定性路由返回）
        must_contain=[r"只读|暂不支持|不能|不支持"],
        # 重要：不能假宣称已发
        must_not_contain=[
            r"已发|已推|已通知|已转发|已经发",
            r"发送成功|推送成功|通知成功",
    ...
    Case(
        name="WS-150: 飞书拒绝 - 推到群变体",
        question="把现在的库存情况推到群里",
        must_use_tools=[],
        must_contain=[r"只读|暂不支持|不能|不支持"],
        must_not_contain=[
            r"已发|已推|已通知",
