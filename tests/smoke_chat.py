...
CASES: List[Case] = [
    ...
    Case(
        ...
        must_contain=[
            # 程度修饰 + 旧（陈/偏/有些/有点/比较/较/老 旧），负向断言排除整族后接非数据
            # 对象名词（款/品/产/版/式/链接/货/物/型）→ "老旧款/老旧产品/旧款" 不算警示。
            r"(?:陈|偏|有些|有点|比较|较|老)旧(?!款|品|产|版|式|链接|货|物|型)"
            # 数据类名词 + 旧（数据/销量/noon/快照 … 旧），同样排除非数据对象后缀。
            r"|(?:数据|销量|noon|快照).{0,8}旧(?!款|品|产|版|式|链接|货|物|型)"
            # 旧 + 数据类名词（旧数据/旧的销量快照）。负向断言同样贴在 "旧" 后，挡掉
            # "旧款的数据 / 旧版口径" 这类（旧族词先接对象名词、再蹭到数据名词）的逃逸。
            r"|旧(?!款|品|产|版|式|链接|货|物|型).{0,3}(?:数据|口径|快照|销量|库存)"
            r"|偏保守|过期|不新鲜|滞后|未更新"
        ],   # 任一**数据陈旧**警示（"旧款/旧品/老旧款/老旧产品"等非数据陈旧词不算）
    ),
    # ─── 时间戳精度（防编精确时间）───
    Case(
