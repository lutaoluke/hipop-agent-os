...
CASES: List[Case] = [
    ...
    # 误报。门2 红队又指出：裸 "旧" 太宽——"旧款 SKU / 旧品 / 旧链接" 这种非陈旧词也会
    # 命中，Agent 没报陈旧却被放行（挖空）。收紧为**数据陈旧上下文的 "旧"**：要么带程度
    # 修饰（陈/偏/有些/有点/比较/较/老 + 旧），要么 "旧" 紧贴数据类名词（旧数据/旧快照/
    # 数据…旧），并用负向断言排除 "旧款/旧品/旧版/旧链接/旧式"。仍要求出现陈旧警示
    # （完全不提、或只提"旧款"等非陈旧词 → 仍红）—— 修门误报又不挖空。
    Case(
        name="用户拒绝刷新（要警示陈旧 + 给答案）",
        question="不用上传 不用刷新 现在就告诉我哪些要补",
        must_contain=[
            r"陈旧|偏旧|有些旧|有点旧|比较旧|较旧|老旧"
            r"|数据.{0,8}旧(?!款|品|版|链接|式)|旧.{0,4}(数据|口径|快照|销量)"
            r"|偏保守|过期|不新鲜|滞后|未更新"
        ],   # 任一**数据陈旧**警示（裸"旧款/旧品"不算）
    ),
    # ─── 时间戳精度（防编精确时间）───
    Case(
