...
# ── round-15 fail-then-pass helper ────────────────────────────────────────────

def _round15_procurement_verifier_would_warn(reply: str) -> bool:
    """round-15 生产 verifier 近似版（876539d），用于证明 round-16 样例改前会漏检。

    与当前 check_procurement_rate_reply 的差异：
    - _DESCRIPTIVE_DENOM_RE 缺少 '以X为计算基准' 和 '计算基数用X' 模式
    - _OLD_15PCT_THRESHOLD_RE 缺少 '解释原因' 和 '不满15%' 模式
    - _PLUS_STILL_IN_KPI_RE 缺少 '计到' 和 '算作' 动词
    """
    topic_re = re.compile(r'采购(?:议价率|折扣率)|(?:议价率|折扣率).{0,10}采购')
    if not topic_re.search(reply):
        return False
    formula_wrong_denom_re = re.compile(
        r'议价差额\s*(?:[÷/]|除以)\s*\(?\s*(?:只用|仅用)?\s*1688.{0,15}(?:标准价|标价|参考价)\s*\)?'
        r'(?!\s*[\+＋加].{0,18}头程)',
        re.IGNORECASE,
    )
    formula_external_denom_re = re.compile(
        r'议价差额\s*(?:[÷/]|除以)\s*\(?\s*'
        r'(?:'
    ...
    # round-15 _DESCRIPTIVE_DENOM_RE (missing 以X为计算基准 and 计算基数用X)
    descriptive_denom_re = re.compile(
        r'分母\s*(?:用|是|为|按|采用|设为)\s*(?:noon|产品|商品|平台).{0,20}(?:售价|价格|价)'
        r'|分母\s*(?:用|是|为|按|采用|设为)\s*(?:销售价|售价|采购价|成交价)'
        r'|分母\s*按\s*.{0,20}(?:售价|价格|价)\s*(?:计算|计)'
        r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价)\s*作为\s*分母'
        r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*作为\s*分母'
        r'|用\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价)\s*做\s*分母'
        r'|用\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*做\s*分母',
        re.IGNORECASE,
    )
    # round-15 _OLD_15PCT_THRESHOLD_RE (missing 解释原因 and 不满15%)
    ...
    # round-15 _PLUS_STILL_IN_KPI_RE (missing 计到 and 算作)
    plus_still_in_kpi_re = re.compile(
        ...
        r'|plus.{0,80}作为\s*(?:采购端|采购|议价)?\s*(?:绩效|KPI|考核)(?:核算|指标|目标)?',
        re.IGNORECASE,
    )
    return bool(
        formula_wrong_denom_re.search(reply)
        or formula_external_denom_re.search(reply)
        or descriptive_denom_re.search(reply)
        or old_15pct_re.search(reply)
        or plus_still_in_kpi_re.search(reply)
    )


# ── round-16 fail-then-pass helper ────────────────────────────────────────────

def _round16_procurement_verifier_would_warn(reply: str) -> bool:
    """round-16 生产 verifier 近似版（2fbca385），用于证明 round-17 样例改前会漏检。

    与当前 check_procurement_rate_reply 的差异：
    - _DESCRIPTIVE_DENOM_RE 缺少 '以noon售价作为计算基准' 模式
    - _PLUS_STILL_IN_KPI_RE 缺少 'plus让利算采购KPI' 的 '算' 动词
    - _PLUS_STILL_IN_KPI_RE 缺少 '采购目标' 这类 KPI 目标同义词
    """
    topic_re = re.compile(r'采购(?:议价率|折扣率)|(?:议价率|折扣率).{0,10}采购')
    if not topic_re.search(reply):
        return False
    ...
    descriptive_denom_re = re.compile(
        ...
        r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价)\s*作为\s*分母'
        r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*作为\s*分母'
        r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价|价格)\s*为\s*(?:计算)?(?:基准|基础|基数)'
        r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*为\s*(?:计算)?(?:基准|基础|基数)'
        r'|(?:计算)?(?:基数|基准)\s*(?:用|是|为|按)\s*(?:noon|产品|商品|平台).{0,20}(?:售价|价格|价)'
        r'|(?:计算)?(?:基数|基准)\s*(?:用|是|为|按)\s*(?:销售价|售价|采购价|成交价)'
        r'|用\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价)\s*做\s*分母'
        r'|用\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*做\s*分母',
        re.IGNORECASE,
    )
    old_15pct_re = re.compile(
        ...
        r'(?:≥|>=|>|＞|大于|高于|超过|达到|达|不低于|不能低于|最低|至少)?\s*'
        r'(?:15\s*[%％]|15\s*(?:个点|个百分点|百分点)|十五\s*(?:个点|个百分点|百分点)?)'
        r'.{0,24}(?:合格|达标|正常|过线|备注|不合格|绩效|说明原因|要说明|解释原因|解释)'
        r'|(?:15\s*[%％]|15\s*(?:个点|个百分点|百分点)|十五\s*(?:个点|个百分点|百分点)?)'
        r'.{0,16}(?:才|即|就|以上|为|算).{0,8}(?:合格|达标|正常|过线)'
        r'|(?:不满|不足)\s*15\s*[%％].{0,15}(?:解释|说明|备注|要解释|要说明)',
        re.IGNORECASE,
    )
    plus_still_in_kpi_re = re.compile(
        r'plus.{0,80}(?<!不)(?:计入|纳入|算入|算进|进入|归入|列入|计到|算作).{0,20}(?:采购端|采购|议价)?(?:绩效|KPI|考核|考核项)'
        r'|plus.{0,80}(?:会|仍|还)\s*(?:计入|纳入|进入|归入).{0,20}(?:议价)?(?:绩效|KPI|考核)'
        r'|plus.{0,80}(?:会|仍|还)\s*纳入.{0,20}(?:采购端|议价)?(?:绩效|KPI|考核)'
        r'|绩效考核时?\s*仍\s*(?:纳入|计入|进入|归入).{0,15}(?:绩效|KPI|考核)'
        r'|要把.{0,5}plus.{0,30}(?:一起)?.{0,5}(?:算进去|算进来|算入|计入|纳入|进入|归入)'
...
def _round17_procurement_verifier_would_warn(reply: str) -> bool:
    """round-17 生产 verifier 近似版（06fae82），用于证明 round-18 样例改前会漏检。

    与 round-18 需要补的差异：
    - _DESCRIPTIVE_DENOM_RE 缺少 "核算基准" 和 "按X作为基准来算" 模式
    - _PLUS_STILL_IN_KPI_RE 缺少 "归到" 动词
    """
    topic_re = re.compile(r'采购(?:议价率|折扣率)|(?:议价率|折扣率).{0,10}采购')
    if not topic_re.search(reply):
        return False
    ...
    descriptive_denom_re = re.compile(
        ...
        r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价|价格)\s*为\s*(?:计算)?(?:基准|基础|基数)'
        r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*为\s*(?:计算)?(?:基准|基础|基数)'
        r'|以\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价|价格)\s*作为\s*(?:计算)?(?:基准|基础|基数)'
        r'|以\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*作为\s*(?:计算)?(?:基准|基础|基数)'
        r'|(?:计算)?(?:基数|基准)\s*(?:用|是|为|按)\s*(?:noon|产品|商品|平台).{0,20}(?:售价|价格|价)'
        r'|(?:计算)?(?:基数|基准)\s*(?:用|是|为|按)\s*(?:销售价|售价|采购价|成交价)'
        r'|用\s*(?:noon\s*(?:平台)?\s*)?(?:产品|商品)?\s*(?:售价|销售价)\s*做\s*分母'
        r'|用\s*noon\s*(?:平台)?\s*.{0,15}(?:价格|售价|价)\s*做\s*分母',
    ...
    plus_still_in_kpi_re = re.compile(
        r'plus.{0,80}(?<!不)(?:计入|纳入|算入|算进|进入|归入|列入|计到|算作|算).{0,20}(?:采购端|采购|议价)?(?:绩效|KPI|考核|考核项|指标|目标)'
        r'|plus.{0,80}(?:会|仍|还)\s*(?:计入|纳入|进入|归入).{0,20}(?:议价)?(?:绩效|KPI|考核|指标|目标)'
        r'|plus.{0,80}(?:会|仍|还)\s*纳入.{0,20}(?:采购端|议价)?(?:绩效|KPI|考核|指标|目标)'
        r'|绩效考核时?\s*仍\s*(?:纳入|计入|进入|归入).{0,15}(?:绩效|KPI|考核|指标|目标)'
        r'|要把.{0,5}plus.{0,30}(?:一起)?.{0,5}(?:算进去|算进来|算入|计入|纳入|进入|归入)'
        r'|plus.{0,80}作为\s*(?:采购端|采购|议价)?\s*(?:绩效|KPI|考核|指标|目标)(?:核算|指标|目标)?',
        re.IGNORECASE,
    )
    return bool(
        formula_wrong_denom_re.search(reply)
...
def _round14_procurement_verifier_would_warn(reply: str) -> bool:
    """round-14 生产 verifier 近似版（c1a4ff2），用于证明 round-15 样例改前会漏检。

    与当前 check_procurement_rate_reply 的差异：
    - _DESCRIPTIVE_DENOM_RE 缺少 '用X做分母' 和 '分母按X计算' 模式
    - _PLUS_STILL_IN_KPI_RE 缺少 '作为...KPI核算' 模式
    """
    topic_re = re.compile(r'采购(?:议价率|折扣率)|(?:议价率|折扣率).{0,10}采购')
    if not topic_re.search(reply):
        return False
    ...
    # round-14 _DESCRIPTIVE_DENOM_RE (missing 用X做分母 and 分母按X计算)
    descriptive_denom_re = re.compile(
    ...
    # round-14 _PLUS_STILL_IN_KPI_RE (missing 作为...KPI核算 pattern)
    plus_still_in_kpi_re = re.compile(
        r'plus.{0,80}(?<!不)(?:计入|纳入|算入|算进|进入|归入|列入).{0,20}(?:采购端|采购|议价)?(?:绩效|KPI|考核)'
        r'|plus.{0,80}(?:会|仍|还)\s*(?:计入|纳入|进入|归入).{0,20}(?:议价)?(?:绩效|KPI|考核)'
        r'|plus.{0,80}(?:会|仍|还)\s*纳入.{0,20}(?:采购端|议价)?(?:绩效|KPI|考核)'
...
def test_bypass34_yong_chanpin_shoujia_zuo_fenmu_old_passes_new_rejects():
    """round-15 fail-then-pass：'用产品售价做分母'应被新 verifier 拦截。

    round-14 _DESCRIPTIVE_DENOM_RE 要求'分母'在'用'前，'用X做分母'语序漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率用产品售价做分母，分子是议价差额。"
        "阈值：3% 不合格，6% 正常。plus折扣不计入采购绩效。"
    )
    assert not _round14_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-14 verifier 应漏检'用产品售价做分母'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
        "Step B：round-15 verifier 应触发采购专项 warns，"
        "但 warns=[]；reply=" + reply
    )
    assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
        "warns 应包含采购专项规则源，实际: " + str(warns)
    )


def test_bypass35_fenmu_an_noon_shoujia_jisuan_old_passes_new_rejects():
    """round-15 fail-then-pass：'分母按 noon平台售价计算'应被新 verifier 拦截。

    round-14 _DESCRIPTIVE_DENOM_RE 不含'按X计算'模式，该语序漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率分母按 noon平台售价计算，分子是议价差额。"
        "阈值：3% 不合格，6% 正常。plus折扣不计入采购绩效。"
    )
    assert not _round14_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-14 verifier 应漏检'分母按noon平台售价计算'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass36_plus_zuowei_caigou_kpi_hesuan_old_passes_new_rejects():
    """round-15 fail-then-pass：'plus补贴作为采购KPI核算'应被新 verifier 拦截。

    round-14 _PLUS_STILL_IN_KPI_RE 不含'作为...KPI核算'动词模式，该说法漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    assert not _round14_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-14 verifier 应漏检'plus补贴作为采购KPI核算'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass37_jisuan_jishu_yong_chanpin_shoujia_old_passes_new_rejects():
    """round-16 fail-then-pass：'计算基数用产品售价'应被新 verifier 拦截。

    round-15 _DESCRIPTIVE_DENOM_RE 不含'计算基数用X'模式，该语序漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率的计算基数用产品售价，分子是议价差额。"
        "阈值：3%不合格，6%正常。plus折扣不计入采购绩效。"
    )
    assert not _round15_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-15 verifier 应漏检'计算基数用产品售价'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
        "Step B：round-16 verifier 应触发采购专项 warns，"
        "但 warns=[]；reply=" + reply
    )
    assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
        "warns 应包含采购专项规则源，实际: " + str(warns)
...
def test_bypass38_yi_noon_shoujia_wei_jisuan_jizhen_old_passes_new_rejects():
    """round-16 fail-then-pass：'以noon平台售价为计算基准'应被新 verifier 拦截。

    round-15 _DESCRIPTIVE_DENOM_RE 不含'以X为计算基准'模式，该语序漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率以noon平台售价为计算基准，分子是议价差额。"
        "阈值3%不合格，6%正常。plus折扣不计入采购绩效。"
    )
    assert not _round15_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-15 verifier 应漏检'以noon平台售价为计算基准'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass39_buman_15pct_jieshi_yuanyin_old_passes_new_rejects():
    """round-16 fail-then-pass：'不满15%要解释原因'旧阈值话术应被新 verifier 拦截。

    round-15 _OLD_15PCT_THRESHOLD_RE 尾关键词不含'解释原因'，且无'不满15%'独立模式。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    assert not _round15_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-15 verifier 应漏检'不满15%要解释原因'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass40_plus_jidao_caigou_kpi_li_old_passes_new_rejects():
    """round-16 fail-then-pass：'plus补贴计到采购KPI里'应被新 verifier 拦截。

    round-15 _PLUS_STILL_IN_KPI_RE 动词列表缺少'计到'，该话术漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率=议价差额÷(1688采购标准价+头程运费分摊)×100%。"
        "阈值3%不合格，6%正常。plus补贴计到采购KPI里。"
    )
    assert not _round15_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-15 verifier 应漏检'plus补贴计到采购KPI里'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass41_plus_suanzuo_caigou_kaohe_xiang_old_passes_new_rejects():
    """round-16 fail-then-pass：'plus补贴算作采购考核项'应被新 verifier 拦截。

    round-15 _PLUS_STILL_IN_KPI_RE 动词列表缺少'算作'，该话术漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率=议价差额÷(1688采购标准价+头程运费分摊)×100%。"
        "阈值3%不合格，6%正常。plus补贴算作采购考核项。"
    )
    assert not _round15_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-15 verifier 应漏检'plus补贴算作采购考核项'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_verifier_round16_bypass_variants_warn():
    """直接 verifier 测试：round-16 新增的 5 类绕过变体必须全部触发 warns。"""
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    for bad_reply in bad_replies:
        warns = check_procurement_rate_reply(bad_reply)
        assert warns, (
            "round-16 verifier 应触发采购专项 warns，"
            "但 warns=[]；reply=" + bad_reply
        )
        assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
            "warns 应包含采购专项规则源，实际: " + str(warns)
        )
...
def test_agent_round16_wires_procurement_specific_warning_for_new_bypasses():
    """round-16 集成测试：round-16 新绕过话术必须进入 agent hallucination_warnings。

    通过 mock LLM provider 路径验证 agent.py 生产接线正确：
    check_procurement_rate_reply() 被调用，hallucination_warnings 包含采购专项拦截，
    而不只是泛化低置信 banner。
    """
    from hipop.server import _provider, agent
    ...
    for bad_reply in bad_replies:
        ...
        warns = result.get("hallucination_warnings") or []
        assert warns, (
            "agent.chat() 应对 round-16 绕过话术触发 hallucination_warnings，"
            "但为空；reply=" + bad_reply
        )
        assert any("采购议价率" in w and "规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
            "hallucination_warnings 应包含采购专项拦截结果，而不是只有泛化低置信 warning；"
...
def test_bypass42_yi_noon_shoujia_zuowei_jisuan_jizhun_old_passes_new_rejects():
    """round-17 fail-then-pass：'以noon售价作为计算基准'应被新 verifier 拦截。

    round-16 _DESCRIPTIVE_DENOM_RE 覆盖了'以noon平台售价为计算基准'，
    但缺少'作为计算基准'和无'平台'的组合，验门人实测漏检。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率以noon售价作为计算基准，分子是议价差额。"
        "阈值3%不合格，6%正常。plus折扣不计入采购绩效。"
    )
    assert not _round16_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-16 verifier 应漏检'以noon售价作为计算基准'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
        "Step B：round-17 verifier 应触发采购专项 warns，"
        "但 warns=[]；reply=" + reply
    )
    assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
        "warns 应包含采购专项规则源，实际: " + str(warns)
...
def test_bypass43_plus_rangli_suan_caigou_kpi_old_passes_new_rejects():
    """round-17 fail-then-pass：'plus让利算采购KPI'应被新 verifier 拦截。

    round-16 _PLUS_STILL_IN_KPI_RE 覆盖 '算入/算进/算作'，但缺少口语动词 '算'。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率=议价差额÷(1688采购标准价+头程运费分摊)×100%。"
        "阈值3%不合格，6%正常。plus让利算采购KPI。"
    )
    assert not _round16_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-16 verifier 应漏检'plus让利算采购KPI'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass44_plus_pingtai_butie_jiru_caigou_mubiao_old_passes_new_rejects():
    """round-17 fail-then-pass：'plus平台补贴计入采购目标'应被新 verifier 拦截。

    round-16 _PLUS_STILL_IN_KPI_RE 只把采购绩效/KPI/考核当成目标词，
    漏掉采购目标这类 KPI 目标同义词。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    reply = (
        "采购议价率=议价差额÷(1688采购标准价+头程运费分摊)×100%。"
        "阈值3%不合格，6%正常。plus平台补贴计入采购目标。"
    )
    assert not _round16_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-16 verifier 应漏检'plus平台补贴计入采购目标'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_verifier_round17_bypass_variants_warn():
    """直接 verifier 测试：round-17 新增的 3 类绕过变体必须全部触发 warns。"""
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    for bad_reply in bad_replies:
        warns = check_procurement_rate_reply(bad_reply)
        assert warns, (
            "round-17 verifier 应触发采购专项 warns，"
            "但 warns=[]；reply=" + bad_reply
        )
        assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
            "warns 应包含采购专项规则源，实际: " + str(warns)
...
def test_agent_round17_wires_procurement_specific_warning_for_new_bypasses():
    """round-17 集成测试：round-17 新绕过话术必须进入 agent hallucination_warnings。"""
    from hipop.server import _provider, agent
    ...
    for bad_reply in bad_replies:
        ...
        warns = result.get("hallucination_warnings") or []
        assert warns, (
            "agent.chat() 应对 round-17 绕过话术触发 hallucination_warnings，"
            "但为空；reply=" + bad_reply
        )
        assert any("采购议价率" in w and "规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
            "hallucination_warnings 应包含采购专项拦截结果，而不是只有泛化低置信 warning；"
...
def test_verifier_round15_bypass_variants_warn():
    """直接 verifier 测试：round-15 新增的 3 类绕过变体必须全部触发 warns。"""
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    for bad_reply in bad_replies:
        warns = check_procurement_rate_reply(bad_reply)
        assert warns, (
            "round-15 verifier 应触发采购专项 warns，"
            "但 warns=[]；reply=" + bad_reply
        )
        assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
            "warns 应包含采购专项规则源，实际: " + str(warns)
...
def test_bypass45_yi_noon_jiage_zuowei_hesuan_jizhun_old_passes_new_rejects():
    """round-18 fail-then-pass：'以noon价格作为核算基准'应被新 verifier 拦截。

    round-17 _DESCRIPTIVE_DENOM_RE 覆盖计算基准/基础/基数，但未覆盖核算基准。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率以noon价格作为核算基准，分子是议价差额。"
        "阈值3%不合格，6%正常。plus折扣不计入采购绩效。"
    )
    assert not _round17_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-17 verifier 应漏检'以noon价格作为核算基准'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
        "Step B：round-18 verifier 应触发采购专项 warns，"
        "但 warns=[]；reply=" + reply
    )
    assert any("规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
        "warns 应包含采购专项规则源，实际: " + str(warns)
...
def test_bypass46_an_chanpin_shoujia_zuowei_jizhun_laisuan_old_passes_new_rejects():
    """round-18 fail-then-pass：'按产品售价作为基准来算'应被新 verifier 拦截。

    round-17 _DESCRIPTIVE_DENOM_RE 覆盖'以X作为基准'和'基准按X'，
    但未覆盖'按X作为基准来算'语序。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率按产品售价作为基准来算，分子是议价差额。"
        "阈值3%不合格，6%正常。plus折扣不计入采购绩效。"
    )
    assert not _round17_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-17 verifier 应漏检'按产品售价作为基准来算'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_bypass47_plus_butie_guidao_caigou_mubiao_old_passes_new_rejects():
    """round-18 fail-then-pass：'plus平台补贴归到采购目标里'应被新 verifier 拦截。

    round-17 _PLUS_STILL_IN_KPI_RE 覆盖归入/进入/计到等动词，但未覆盖'归到'。
    """
    from hipop.rules.procurement_rate import check_procurement_rate_reply

    reply = (
        "采购议价率=议价差额÷(1688采购标准价+头程运费分摊)×100%。"
        "阈值3%不合格，6%正常。plus平台补贴归到采购目标里。"
    )
    assert not _round17_procurement_verifier_would_warn(reply), (
        "Step A fail-then-pass：round-17 verifier 应漏检'plus平台补贴归到采购目标里'话术；"
        "reply=" + reply
    )
    warns = check_procurement_rate_reply(reply)
    assert warns, (
...
def test_verifier_round18_bypass_variants_warn():
    """round-18 直接 verifier：验门人 round-17 新绕过必须触发专项 warns。"""
    from hipop.rules.procurement_rate import check_procurement_rate_reply
    ...
    for bad_reply in bad_replies:
        assert not _round17_procurement_verifier_would_warn(bad_reply), (
            "Step A fail-then-pass：round-17 verifier 应漏检此自然绕过样例；"
            "reply=" + bad_reply
        )
        warns = check_procurement_rate_reply(bad_reply)
        assert warns, (
...
def test_agent_round18_wires_procurement_specific_warning_for_new_bypasses():
    """round-18 agent-path：验门人 round-17 新绕过必须进入 hallucination_warnings。"""
    from hipop.server import _provider, agent
    ...
    for bad_reply in bad_replies:
        ...
        warns = result.get("hallucination_warnings") or []
        assert warns, (
            "agent.chat() 应对 round-18 新绕过触发 hallucination_warnings，"
            "但为空；reply=" + bad_reply
        )
        assert any("采购议价率" in w and "规则源: hipop/rules/procurement_rate.py" in w for w in warns), (
            "hallucination_warnings 应包含采购专项拦截结果，而不是只有泛化低置信 warning；"
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        ("test_bypass33_guiru_caigou_kaohe_old_passes_new_rejects",
         test_bypass33_guiru_caigou_kaohe_old_passes_new_rejects),
        # ── round-15 oracle bypasses (用X做分母/分母按X计算/作为采购KPI核算) ──
        ("test_bypass34_yong_chanpin_shoujia_zuo_fenmu_old_passes_new_rejects",
         test_bypass34_yong_chanpin_shoujia_zuo_fenmu_old_passes_new_rejects),
        ("test_bypass35_fenmu_an_noon_shoujia_jisuan_old_passes_new_rejects",
         test_bypass35_fenmu_an_noon_shoujia_jisuan_old_passes_new_rejects),
        ("test_bypass36_plus_zuowei_caigou_kpi_hesuan_old_passes_new_rejects",
         test_bypass36_plus_zuowei_caigou_kpi_hesuan_old_passes_new_rejects),
        # ── round-16 oracle bypasses (计算基数用X/以X为计算基准/不满15%解释/计到KPI/算作考核项) ──
        ("test_bypass37_jisuan_jishu_yong_chanpin_shoujia_old_passes_new_rejects",
         test_bypass37_jisuan_jishu_yong_chanpin_shoujia_old_passes_new_rejects),
        ("test_bypass38_yi_noon_shoujia_wei_jisuan_jizhen_old_passes_new_rejects",
         test_bypass38_yi_noon_shoujia_wei_jisuan_jizhen_old_passes_new_rejects),
        ("test_bypass39_buman_15pct_jieshi_yuanyin_old_passes_new_rejects",
         test_bypass39_buman_15pct_jieshi_yuanyin_old_passes_new_rejects),
        ("test_bypass40_plus_jidao_caigou_kpi_li_old_passes_new_rejects",
         test_bypass40_plus_jidao_caigou_kpi_li_old_passes_new_rejects),
        ("test_bypass41_plus_suanzuo_caigou_kaohe_xiang_old_passes_new_rejects",
         test_bypass41_plus_suanzuo_caigou_kaohe_xiang_old_passes_new_rejects),
        # ── round-17 oracle bypasses (作为计算基准/plus让利算KPI/采购目标) ──
        ("test_bypass42_yi_noon_shoujia_zuowei_jisuan_jizhun_old_passes_new_rejects",
         test_bypass42_yi_noon_shoujia_zuowei_jisuan_jizhun_old_passes_new_rejects),
        ("test_bypass43_plus_rangli_suan_caigou_kpi_old_passes_new_rejects",
         test_bypass43_plus_rangli_suan_caigou_kpi_old_passes_new_rejects),
        ("test_bypass44_plus_pingtai_butie_jiru_caigou_mubiao_old_passes_new_rejects",
         test_bypass44_plus_pingtai_butie_jiru_caigou_mubiao_old_passes_new_rejects),
        # ── round-18 oracle bypasses (核算基准/按X作为基准/归到采购目标) ──
        ("test_bypass45_yi_noon_jiage_zuowei_hesuan_jizhun_old_passes_new_rejects",
         test_bypass45_yi_noon_jiage_zuowei_hesuan_jizhun_old_passes_new_rejects),
        ("test_bypass46_an_chanpin_shoujia_zuowei_jizhun_laisuan_old_passes_new_rejects",
         test_bypass46_an_chanpin_shoujia_zuowei_jizhun_laisuan_old_passes_new_rejects),
        ("test_bypass47_plus_butie_guidao_caigou_mubiao_old_passes_new_rejects",
         test_bypass47_plus_butie_guidao_caigou_mubiao_old_passes_new_rejects),
        ...
        ("test_verifier_round14_bypass_variants_warn",
         test_verifier_round14_bypass_variants_warn),
        ("test_verifier_round15_bypass_variants_warn",
         test_verifier_round15_bypass_variants_warn),
        ("test_verifier_round16_bypass_variants_warn",
         test_verifier_round16_bypass_variants_warn),
        ("test_agent_round16_wires_procurement_specific_warning_for_new_bypasses",
         test_agent_round16_wires_procurement_specific_warning_for_new_bypasses),
        ("test_verifier_round17_bypass_variants_warn",
         test_verifier_round17_bypass_variants_warn),
        ("test_agent_round17_wires_procurement_specific_warning_for_new_bypasses",
         test_agent_round17_wires_procurement_specific_warning_for_new_bypasses),
        ("test_verifier_round18_bypass_variants_warn",
         test_verifier_round18_bypass_variants_warn),
        ("test_agent_round18_wires_procurement_specific_warning_for_new_bypasses",
         test_agent_round18_wires_procurement_specific_warning_for_new_bypasses),
