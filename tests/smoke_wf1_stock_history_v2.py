...
def main():
    ...
    for bad in (None, "", "today", "2026/05/30", "20260530"):
        raised = False
        try:
            stock_history.run_v2(TENANT, as_of_date=bad)
        except ValueError:
            raised = True
        assert raised, f"as_of_date={bad!r} 应 raise，而不是回落到假业务日"
    # 缺失业务日时绝不落库
    ...
    print("✓ as_of_date 来自运行参数：缺失/非法红灯 raise，不取 today、不从 imported_at 反推")
